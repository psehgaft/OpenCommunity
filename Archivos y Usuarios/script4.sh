...
while IFS=$'\t' read NOMBRE ID_USUARIO GRUPOS_RAW; do
    ...
    if [ "$ES_ALUMNO" == "Y" -a "$ES_MAESTRO" != "Y" ]; then
        mkdir -p $HOME_USUARIO/grupo
    fi

    #Estableciendo propietarios y permisos

    chown -R $ID_USUARIO:$GRUPO_PRINCIPAL $HOME_USUARIO/ /compartidos/$ID_USUARIO
    chmod a+rX /compartidos/$ID_USUARIO
done
)
